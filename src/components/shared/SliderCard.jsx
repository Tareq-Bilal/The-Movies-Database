...
const SliderCard = ({ data, theme = 'light' }) => {
  ...
  return (
    <>
      <div className={styles.sliderCard}>
        ...
        >
          <h3
            className={`${styles.movieTitle} ${theme === 'dark' ? styles.titleWithBackground : ''}`}
          >
            {data?.title || data?.name || 'Unknown Title'}
          </h3>
          <p
            className={`${styles.movieDate} ${theme === 'dark' ? styles.dateWithBackground : ''}`}
          >
            {formatDate(data?.release_date || data?.first_air_date || 'No Date')}
          </p>
        </div>
      </div>
    </>
